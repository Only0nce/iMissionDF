...
QString NetworkMng::readLine(const QString &fileName) {
...
void NetworkMng::getIPAddress(const QString &netWorkCard) {
    QString prog = "/bin/bash"; //shell
    QStringList arguments;
    ...
    qDebug() << "getIPAddress netWorkCardAddr" << netWorkCardAddr;
    arguments.clear();
}
bool NetworkMng::getLinkDetected(const QString &networkCard) {
...
bool NetworkMng::getStorage(const QString &deviceName) {
...
QString NetworkMng::getAddress(const QString &netWorkCard) {
    QString networkInfo;
    QString prog = "/bin/bash"; //shell
    QStringList arguments;
...
void NetworkMng::resetNetwork() {
    ...
    arguments << "-c" << QString("ifconfig %1 down").arg(phyName);
    getAddressProcess -> start(prog, arguments);
    getAddressProcess -> waitForFinished();

    arguments.clear();
    arguments << "-c" << QString("ifconfig %1 up").arg(phyName);
    getAddressProcess -> start(prog, arguments);
    getAddressProcess -> waitForFinished();

    arguments.clear();
    arguments << "-c" << QString("netplan apply");
    getAddressProcess -> start(prog, arguments);
    getAddressProcess -> waitForFinished();

    qDebug() << "networking restarted";
    //    getIPAddress(phyName);
    getAddress(phyName);
}

void NetworkMng::resetNetworknmcli() {
    QString prog = "/bin/bash"; //shell
    QStringList arguments;
    qDebug() << "networking restarting";
    arguments << "-c" << QString("nmcli connection down %1").arg(phyName);
    getAddressProcess -> start(prog, arguments);
    getAddressProcess -> waitForFinished();

    arguments.clear();
    arguments << "-c" << QString("nmcli connection up %1").arg(phyName);
    getAddressProcess -> start(prog, arguments);
    getAddressProcess -> waitForFinished();
...
void NetworkMng::setNTPServer(const QString &ntpServer) {
...
void NetworkMng::setDHCPIpAddr(const QString &netWorkCard) {
    QString filename = "/etc/netplan/00-installer-config.yaml";
...
void NetworkMng::initCheckCard(const QString &phyNetworkName) {
...
void NetworkMng::checkCard(const QString &phyNetworkName) {
    if (phyNetworkName == "eth0") {
...
void NetworkMng::setDHCPIpAddr3(const QString &phyNetworkName) {
...
void NetworkMng::connmanSetDHCP(const QString &phyNetworkName) {
    QString command;
    QString macAddress;
...
int NetworkMng::calPrefix(const QString &mask) {
    uint32_t longMask = 0;
    int prefix = 24;
...
bool NetworkMng::checkAddress(const QString &address) {
...
void NetworkMng::setStaticIpAddr3(QString ipaddr, QString netmask,QString gateway,QString dns1,QString dns2,QString phyNetworkName) {
    phyName = phyNetworkName;
    QString connectionName = getCurrentNetworkName(phyNetworkName);
    qDebug() << "connectionName" << connectionName << ipaddr << gateway;
    if (connectionName == "") return;
