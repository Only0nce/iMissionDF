...
static bool vpnFeatureEnabledFromConfig()
{
    ...
    // Backward-compatible default for deployments created before VPN settings.
...
QVariantMap NetworkController::vpnStatus()
{
    ...
    if (!ok) {
        ...
        result[QStringLiteral("message")] = err.trimmed().isEmpty()
    ...
    if (activeQueryOk) {
        ...
        for (const QString &line : lines) {
            ...
            if (type != QStringLiteral("vpn") && type != QStringLiteral("wireguard"))
                continue;
            QVariantMap row;
            row[QStringLiteral("name")] = f.value(0).trimmed();
            row[QStringLiteral("uuid")] = f.value(1).trimmed();
            row[QStringLiteral("type")] = type;
            row[QStringLiteral("device")] = f.value(3).trimmed();
            activeByUuid.insert(row.value(QStringLiteral("uuid")).toString(), row);
        }
    }
    ...
    for (const QString &line : lines) {
        ...
        const QString runtimeDevice = active ? activeRow.value(QStringLiteral("device")).toString().trimmed() : QString();
    ...
    result[QStringLiteral("state")] = activeProfiles.isEmpty() ? QStringLiteral("disconnected") : QStringLiteral("connected");
    ...
    if (!activeQueryOk && !activeErr.trimmed().isEmpty())
        result[QStringLiteral("message")] = QStringLiteral("VPN profile query warning: %1").arg(activeErr.trimmed());
...
void NetworkController::requestVpnPublicIp()
{
    ...
    request.setRawHeader("User-Agent", "iScanMR10/NET-VPN");
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply, manager]() {
        ...
        if (reply->error() == QNetworkReply::NoError) {
            ...
            ok = address.setAddress(ip);
            message = ok ? QStringLiteral("Public IP refreshed") : QStringLiteral("Public IP service returned an invalid address");
            if (!ok) ip.clear();
        } else {
            message = reply->errorString().trimmed();
            if (message.isEmpty()) message = QStringLiteral("Unable to resolve public IP");
        }
...
void NetworkController::setVpnEnabled(bool enabled)
{
    ...
    QThread *thread = QThread::create([self, enabled]() {
        ...
        if (!saved) {
            const QString msg = saveMessage.isEmpty() ? QStringLiteral("Failed to save VPN enable state") : saveMessage;
            if (!self) return;
        ...
        if (!enabled) {
            ...
            const bool queryOk = runProcessBlocking(QStringLiteral("nmcli"),
                                                     {QStringLiteral("-t"), QStringLiteral("-f"), QStringLiteral("UUID,TYPE"),
                                                      QStringLiteral("connection"), QStringLiteral("show"), QStringLiteral("--active")},
                                                     &activeOut, &activeErr, 10000);
            if (!queryOk) {
                ok = false;
                failures << (activeErr.trimmed().isEmpty() ? QStringLiteral("Unable to enumerate active VPN tunnels") : activeErr.trimmed());
            } else {
                for (const QString &line : activeOut.split('\n', QString::SkipEmptyParts)) {
                    ...
                    if (uuid.isEmpty() || (type != QStringLiteral("vpn") && type != QStringLiteral("wireguard")))
                        continue;
                    QString out, err;
                    if (!runProcessBlocking(QStringLiteral("nmcli"),
                                            {QStringLiteral("connection"), QStringLiteral("down"), QStringLiteral("uuid"), uuid},
                                            &out, &err, 45000)) {
                        ok = false;
                        failures << (!err.trimmed().isEmpty() ? err.trimmed() : out.trimmed());
                    }
                }
            }
        }
        const QString message = enabled
            ? QStringLiteral("VPN enabled. Select a profile and press Connect.")
            : (ok ? QStringLiteral("VPN disabled and active tunnels disconnected")
                  : QStringLiteral("VPN disabled, but runtime cleanup reported: %1").arg(failures.join(QStringLiteral(" | "))));
        if (!self) return;
...
void NetworkController::setVpnConnectionActive(const QString &uuid, bool active)
{
    ...
    if (cleanUuid.isEmpty()) {
        emit vpnOperationFinished(active ? QStringLiteral("connect") : QStringLiteral("disconnect"), false,
                                  QStringLiteral("VPN profile UUID is empty"));
        return;
    }
    ...
    QThread *thread = QThread::create([self, cleanUuid, active]() {
        if (active && !vpnFeatureEnabledFromConfig()) {
            ...
            QMetaObject::invokeMethod(self, [self, msg]() {
                if (self) emit self->vpnOperationFinished(QStringLiteral("connect"), false, msg);
            }, Qt::QueuedConnection);
            return;
        }
        ...
        const QString msg = ok
            ? (active ? QStringLiteral("VPN connect command completed; verifying tunnel state")
                      : QStringLiteral("VPN disconnect command completed; verifying tunnel state"))
            : (detail.isEmpty() ? QStringLiteral("VPN command failed") : detail);
        if (!self) return;
